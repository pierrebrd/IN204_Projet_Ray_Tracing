...
std::tuple<float, float, float> determineLightIntensity(Point_3D intersection, Object* intersected_object, std::vector<Light>* lights, std::vector<Object*>* objects, Ray incidentRay) {
    ...
    return minTuple(lightIntensityReflection + lightIntensityShadow, {1,1,1});
}


std::tuple<float, float, float> compute_color(Ray myRay, std::vector<Object*>* objects, std::vector<Light>* lights) {
    ...
    if (!nextIntersect.has_value()) {
        ...
        std::tuple<float, float, float> lightIntensity = determineLightIntensity(intersection, intersected_object, lights, objects, myRay);
...
void image_creator(Camera cam, std::vector<Object*>* objects, std::vector<Light>* lights, std::string filename) {
    ...
    for (unsigned i = 0; i < cam.get_resolution_height(); i++) {
        for (unsigned j = 0; j < cam.get_resolution_width(); j++) {
            ...
            std::tuple<float, float, float> colors = compute_color(initial_Ray, objects, lights);
